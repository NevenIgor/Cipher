# -*- coding: utf-8 -*-
# -*- developed by: Nevenchenko I.E. -*-
from time_tracer import tracer
from PyQt5 import QtCore


class BlockCipher(QtCore.QObject):
    signal = QtCore.pyqtSignal(int)

    def __init__(self, parent=None):
        super().__init__(parent=parent)

        self.Lvect = bytes((148, 32, 133, 16, 194, 192, 1, 251, 1, 192, 194, 16, 133, 32, 148, 1))

        self.L_k = bytes((148, 32, 133, 16, 194, 192, 1, 251))

        self.Pi = bytes((252, 238, 221, 17, 207, 110, 49, 22, 251, 196, 250, 218, 35, 197, 4, 77, 233, 119, 240,
        ...
        self.Pi_reverse = bytes((0xA5, 0x2D, 0x32, 0x8F, 0x0E, 0x30, 0x38, 0xC0, 0x54, 0xE6, 0x9E, 0x39, 0x55, 0x7E,
    ...
    def encrypt(self, data, key, mode):
        ...
        if mode == 'ECB':
            ...
            fives = [num_blocks // 100 * i for i in range(100)]
            for i in range(num_blocks):
                if i in fives:
                    self.signal.emit(fives.index(i))
...
class Gamma(object):
    def __init__(self, iv, masterkey):
        ...
        self.keys = self.bc.create_keys(self.master_key)

        self.gamma = b''

    def gen(self, length):
        ctr = self.iv + bytes(8)
        while len(self.gamma) < length:
            self.gamma += self.bc.round(self.keys, ctr)
            ctr = int.to_bytes(int.from_bytes(ctr, 'big') + 1, 16, 'big')
            self.master_key = self.keys[8] + self.keys[9]
            self.keys = self.bc.create_keys(self.master_key)


if __name__ == '__main__':
    bc = BlockCipher()
    bc.encrypt(open('1.jpg', 'rb').read(), bytes.fromhex('00' * 32), 'CBC')
